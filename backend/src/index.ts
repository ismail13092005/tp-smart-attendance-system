...
// Health check — must respond before DB is ready so Railway healthcheck passes
...
async function start() {
  // 1. Start HTTP server first — Railway healthcheck hits /health immediately
  ...
  // 2. Connect to database after server is already up
  try {
    await initDatabase();
    logger.info('Database connected');
  } catch (err) {
    logger.error('Database connection failed — app still running', err);
    return; // Don't start schedulers if DB failed
  }
